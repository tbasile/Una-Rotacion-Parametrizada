...
Una forma podría ser haciendo más énfasis en lo de circuitos 1PR
y que lo de canales de Pauli sea como un ejemplo particular de cómo se puede usar eso. Claro que eso significaría cambiar demasiado el artículo
 (hasta habría que cambiar el título ) y no sé si eso sea factible. Quizá se puede hacer
 más énfasis en lo de 1PR sin cambiar taaanto el orden del artículo\\
Igual la verdad que estoy perdido con qué contestar a este artículo}
...
Here, our systems always consist of some number $N$ of qubits.
The qubits can be divided into the ones to which we will apply the quantum channel, or ``principal qubits'' and the ones that help us complete said operation, or ``ancilla qubits''.
The circuit implements a unitary operator $U$ and then some or all of the qubits are measured.
} 
...
{\color{green}
\subsection{Two-qubit Pauli channels}
...
qubits and $2N$ ancilla qubits, on which it would be necessary to create the state $\sum_{\vec{\gamma}} b_{\vec{\gamma}} |\vec{\gamma}\rangle$.
...
\begin{align}
\varepsilon (\rho) = (1-p) \rho + \dfrac{p}{3} (I \otimes \sigma_z) \rho (I \otimes \sigma_z) +  (\sigma_z \otimes I) \rho (\sigma_z \otimes I) +  (\sigma_z \otimes \sigma_z) \rho (\sigma_z \otimes \sigma_z),
\label{2qbit-c}
\end{align}
...
the two principal qubits. Then, we calculated the diamond fidelity for different values of $p$ and
...
From theorem 1, we conclude that $R$ can be taken in general as a $\sigma_3$ rotation of angle $2s(p)$ applied to the last qubit and controlled by some of the other ones. 
To prove the result, we will apply $U(p)$ to $|j\rangle$ by first applying $B$,
...
the curve of states described in \eref{eq:curve-states}, \revnote{to see what?} \tbnote{Ya cambié la oración anterior} we can rewrite the expression of $B|0\rangle$ by 
...
Now, defining $A$ and $B$ as in  \eref{eq:BA} for the particular
...
\end{pmatrix}
\end{align*}
where the first column of $A$ and the
...
and $20$ repetitions for each value of $p$.
...
& := |c\rangle + e^{is} |a\rangle + e^{-is} |b\rangle,
\end{align*}
...
obtaining as a result the fidelities shown in \fref{FigS2q}.
...
dynamical maps and the we ran the resulting circuit in ibmq-lima with the same
specifications as for the other examples
and the results are shown in \fref{FigS2q}.
...
\caption{
{\bf Fidelitis for dynamical maps.} Results of the fidelities obtained when simulating the three dynamical maps considered (depolarizing, parabolic and bit flip). 
The implementation was done on IBM's ibmq-lima quantum computer.
We can see that just as in \fref{Fig3}, fidelities are high for points close to the depolarizing channel 
...
by using parametrized quantum circuits \revnote{imprecise}.
...
such as those used for quantum machine learning. \revnote{citation required here}
