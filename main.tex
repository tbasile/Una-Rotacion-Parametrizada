...
where the rotation is around $\sigma_3$ and
...
swap gates to convert the rotation
...
and a phase $e^{is}$ to $|1\rangle$, the result is:
\begin{small}
\begin{eqnarray}
...
For 1PR circuits, the ancilla state has to be created with only one parametrized curve. Then, the theorem for 1PR circuits implies that a map
\begin{eqnarray}
...
maps of one qubit that can be implemented with a 1PR circuit:
\begin{itemize}
...
To implement this dynamical map using the circuit presented in \fref{fig:1PR-circuit}, it is necessary to create the state
\begin{eqnarray}
