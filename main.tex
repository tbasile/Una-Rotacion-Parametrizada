...
The state's evolution is unitary and given by Schrodinger's equation~\cite{Rieffel}.
However, in real-world situations, quantum systems are usually open, 
which means that they interact with their environment~\cite{Breuer}. 
For instance, the system's state may become entangled with the environment,
leading to a loss of information about the system's state over time.
...
it must satisfy two conditions: $\tr(\rho) = 1$ and $\rho$ is positive semi-definite,
...
describe the evolution of open quantum systems, such that $\rho \rightarrow \mathcal{E}(\rho)$.
...
system is uniquely associated with a point in a unit sphere. \cpnote{explicar un poco mejor. Me parece que para el publico vale 
...
have to be non-negative real numbers such that 
...
\tbnote{Puedo cambiar la fidelidad por otras distancias, como la diamond norm,
pero estoy buscando cuál conviene más.}}w
