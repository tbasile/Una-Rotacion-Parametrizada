...
Furthermore, we can exhibit a 1PR circuit to create any of these curves of states.
To do it, we can use the circuit of \fref{fig:1PR-circuit} and set the parametrized
rotation $R$ on the last qubit to be controlled by all the other qubits..
Moreover, we define the matrices $A$ and $B$ as:
\begin{align*}
B = (\vec{d}, *,*, \cdots,*), \; 
A = (* , \cdots, *, \vec{c}/|\vec{c}|, \vec{b}/|\vec{b}|, \vec{a}/|\vec{a}|),
\end{align*}
where the column vector $\vec{d}$ is defined as 
...
and applying the circuit (whose matrix is $ARB$), some direct calculations lead us  to 
