...
% }}}
\subsection{Simulation of a two-qubit Pauli channel} \label{2qbit} % {{{
% {\color{green}
...
However, the quantum computer which we are using has only \cpnote{cuantos qubits 
tiene esa compu?}.
does not have that many qubits,
We can still simulate a two qubit Pauli channel that does not apply all 
two-qubit Pauli matrices to the system\cpnote{that applies at most tantos operaciones 
de Pauli}.
...
for the 2-qubit dephasing channel of \eref{2qbit-c}. The state on the two
...
poenr el circuito arriba y la fidelidad abajo. TAbien describir ambas cosas. } }
\label{2qbit}
\end{figure} % }}}
...
It stays at a value of around $0.75$, and it is biggest around $p=0$,
corresponding to the identity channel\cpnote{Una descripcion mas precisa de
estos valores por favor.}
% }
...
\tbnote{Given this condition, we considered the space of Pauli maps that are
...
However, some paradigmatic maps as the dephasing one we studied for 2 qubits in
section \ref{2qbit} will still be possible to do with a 1PR circuit (for this
case, this can be seen by noticing that said dephasing channel in
\eref{2qbit-c} has the same form as the depolarizing channel of \eref{eq:depo},
which we proved to be possible to implement with a 1PR circuit).}
\cpnote{Reescribe lo anterior sin hacer referencias a ecuaciones o secciones.}
