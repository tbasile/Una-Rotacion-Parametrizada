...
Finally, we consider Pauli dynamical maps, which
are curves of Pauli channels parametrized by a variable.

%Quitarrrr:
Then, we propose a quantum circuit that simulates N-qubit Pauli channels
and show the results of implementing the circuit for $N=1$ on
a quantum computer. 
% }}}
...
In quantum mechanics, a closed system is represented by a state vector in a 
Hilbert space $\mathcal{H}$. 
The state's evolution is given by Schrodinger's
equation and is always unitary.
However, in real-world situations, 
quantum systems are usually open and interact with their environment in various complicated ways. 
For example, the system's state may become entangled with the environment,
which can lead to loss of information of the system's state over time.

Instead of vectors in the Hilbert space $\mathcal{H}$,
open systems are described by matrices $\rho$ acting on $\mathcal{H}$
called density matrices and they include information about 
the system's interaction with the environment.
To be physically valid, a density matrix $\rho$
has to satisfy the two following conditions:
1. $\tr(\rho) =1$ and 2. $\rho$ is positive semi-definite $(\rho \geq 0)$.

Given that, we can finally define quantum channels, which
describe the time evolution of open quantum systems.
Quantum channels are the most general linear operations
that a quantum system (which is represented 
by a density matrix $\rho$) can undergo independently of its
...
This is required to ensure that even if $\rho$ is entangled with another system,
applying $\mathcal{E}$ to $\rho$ while doing nothing to the
 other system still results in  a positive semidefinite state. 

Given a possible quantum channel $\mathcal{E}$, 
the condition of trace preservation is straightforward to examine but complete positivity is not as simple.
To test complete positivity of a quantum channel, 
Jamiołkowski and Choi~\cite{choi,jamil} developed a simple algorithm.  
...
type of channel for $N$-qubit systems called Pauli channels.
First we define a Pauli channel on a one-qubit system,
whose most general single-qubit density
...
a vector $\vec{r}= (r_1,r_2,r_3)$ inside a unit sphere known as the Bloch sphere. \cpnote{explicar un poco mejor. Me parece que para el publico vale 
la pena ser un poco mas explicito. pero no estoy muy seguro. }. 
A Pauli channel is defined as an operation that
applies each Pauli matrix $\sigma_{\gamma}$ to
the system with probabilities $k_{\gamma}$ with $\gamma = 0,1,2,3$. 
...
\end{eqnarray} 
where of course the probabilities $k_{\gamma}$ of applying $\sigma_{\gamma}$
...
\cpnote{en todo hay que mejorar un poco la redaccion. Aca me gustaria que 
se interpretara. Creo qe es mas que atenuacion de los componentes, o es solo atenuacion? 
en todo caso ver mejor como va la cosa. }

Apart from this interpretation of Pauli channels,
we can find another one by seeing how the channel acts on the 
density matrix of eq. (\ref{ec: Density Matrix}).
To do it, we substitute eq. (\ref{ec: Density Matrix}) in eq. (\ref{ec: Pauli channel 1 qbit}):
\begin{equation}
...
which leads to
\begin{equation}
\label{ec: rho-transformada}
\mathcal{E}(\rho) = \dfrac{1}{2} \sum_{\alpha} \left(\sum_{\gamma} A_{\alpha, \gamma} k_{\gamma} \right) r_{\alpha} \sigma_{\alpha}.
\end{equation}
Equation \ref{ec: rho-transformada} gives us another interpretation of Pauli channels.
Pauli channels take each component $r_{\alpha}$
of the density matrix and multiply them
by $\sum_{\gamma} A_{\alpha,\gamma} k_{\gamma}$, 
that is:
\begin{equation}
\label{ec: multipliers}
...
we get that $k_{\gamma} = \dfrac{1}{4} \sum_{\gamma} A_{\gamma,\alpha} \tau_{\gamma}$.
Then, using that $k_{\gamma} \geq 0$ and $\sum_{\gamma} k_{\gamma} = 1$,
we get the following conditions on the multipliers:
\begin{eqnarray}
\label{eq: conditions-tetrahedron}
1+\tau_i -\tau_j - \tau_k \geq 0, \\
...
attenuated and possibly flipped sign.

Pauli channels can be used to model noise....
...
the operators $\sigma_{\vec{\gamma}}$ to $\rho$ with probabilities $k_{\vec{\gamma}}$
...
that $k_{\vec{\gamma}}$ are non-negative real numbers such 
that $\sum_{\vec{\gamma}} k_{\vec{\gamma}}=1$.

\textbf{Interpretación de multiplicadores}

\cpnote{Creo qeu toca motivar, conectar con cosas conocidas y en general no ser tan seco}
...
\cpnote{esto está un poco desordenado y a un ritmo creo qeu inapropiado por lo rapido}

We are interested in constructing a quantum circuit that can implement an
arbitrary Pauli channel on a system of $N$ qubits. 
To do it, we remember that a Pauli channel as in eq.~(\ref{ec: pauli channel N-qubt})
is interpreted as a transformation that applies
...
The general idea is to create a state in the ancilla qubits that can then
be used to apply the operators $\sigma_{\vec{\gamma}}$
to the system.
...
\lstick{ } & & & & & &\\
\lstick{$aq_0$} & \qw & \gate[wires=7,nwires=5]{\text{Create the $2N$ qubits state $\sum_{\vec{\gamma}} b_{\vec{\gamma}}   |\vec{\gamma} \rangle$}} & \qw &  \ctrl{-5} & \qw & \rstick[wires=7]{$|\vec{\gamma} \rangle$} &  & & &\\
\lstick{$aq_1$} & \qw & & \qw &  \ctrl{-6} &\qw & & & & & \\
\lstick{$aq_2$} &\qw & & \qw &  \ctrl{-7} & \qw & & & & &\\
\lstick{$aq_3$} &\qw & & \qw &  \ctrl{-8} & \qw & & & & &\\
\vdots  &\vdots & & \vdots & &  \vdots & & & & &\\
\lstick{$aq_{2N-2}$}& \qw &  &\qw  & \ctrl{-10} & \qw & & & & & \\
\lstick{$aq_{2N-1}$}& \qw & & \qw  &  \ctrl{-11} & \qw & & & & &\\
\end{quantikz}
\caption{\textbf{\tbnote{Hay que quitar esta imagen del texto al entregar el artículo y ponerla en un archivo a parte de tipo TIFF or EPS. Hay que dejar el figure solamente con la caption y label.}} {\bf Circuit for an $N$-qubit Pauli channel.} Circuit to implement an $N$-qubit Pauli channel 
