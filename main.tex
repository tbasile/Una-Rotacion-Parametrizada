...
\tbnote{versión muy preliminar de la intro, para ir teniendo algo escrito de una vez. }\\
...
\langle a |c\rangle =
\langle b |c\rangle = 0
% , |b\rangle, |c\rangle \; \text{orthogonal states},
%  \text{and} \; a\rangle, |b\rangle, |c\rangle \; \text{orthogonal states},
...
Given this condition \cpnote{which?}, the curve of states of \eref{eq: parametrized state}  needed 
...
and $\langle a| a\rangle + \langle b| b\rangle + \langle c| c\rangle = 1$ \cpnote{redundante, refierete mejor a la ecuacion anterior, a la 24 pues. Ahora, también creo qeu la pedendencia
con $p$ no está clara del lado derecho. Quizpa si se escribe $s(p)$ en vez de 
...
Valdez me va a pasar la cita. De hecho pordemos leer el articulo para inspiracion y citar}
...
\end{align*}
with $s= \arcsin(\sqrt{p})$\cpnote{Que tal si lo dejamos como $\sin s= \sqrt{p}$? Si estas de 
acuerdo igual cambia los siguientes}.
