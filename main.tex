...
\tbnote{Ya puse la puntuación y lo puse en términos de arctan.
La suma de arcotangentes se puede juntar en una 
sola arctan, pero con un argumento que ya queda muy complicado y 
...
to simulate them. \cpnote{la computadora no la usamos para samplear sino solo para simular. Modificar
la frase.}
\tbnote{Ya.}  For each of these channels, we used quantum process
...
{\color{orange}  We took a sample of one-qubit Pauli channels
...
{\color{orange} The calculation of this norm is done using the semi-definite program from reference \cite{Watrous}.}
\cpnote{Esa definicion esta bonita y me gusta, pero supongo qeu no es asi somo la 
calculas. } 
\tbnote{La calculo usando una función en la librería de Qiskit
que se basa en la referencia que puse,
pero la verdad no sé bien cómo funciona.}
...
\tbnote{Va, ya puse directo con la $f$ y quité lo de psucc}

{\color{orange} When the two channels are the same,
...
with $1$ indicating that the channels are completely
equal and $0$ the opposite.
}
...
$p_{succ}$ para que se cumpla lo de que $0$ es lo peor y $1$ lo mejor.} Results of the diamond fidelities as defined in \eref{eq:diamond-fid} for a sample of Pauli channels in the tetrahedron.
Notice that channels close to the center of the tetrahedron have high fidelities,
while those close to the borders do not. 
Moreover, we show the results for cuts of the tetrahedron at different values of $\tau_3$.}
\label{fig:fids}
\end{figure}
