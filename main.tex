...
which are described by the theory of quantum channels~\cite{cleve,Zanetti,Xin,Wei}. \tbnote{Cita a Andrea}
...
and $\langle a| a\rangle + \langle b| b\rangle + \langle c| c\rangle = 1$}
 \cpnote{redundante, refierete mejor a la ecuacion anterior, a la 24 pues. Ahora, también creo qeu la pedendencia
con $p$ no está clara del lado derecho.}

\tbnote{Va, algo así (quitar lo 
rojo, dejar lo morado):}

{\color{purple} where $|a\rangle,|b\rangle,|c\rangle$ fulfill
the conditions of \eref{eq:conditions-vecs}.}

\cpnote{
 Quizpa si se escribe $s(p)$ en vez de 
$s$ es mejor, pero piensale tambien . } 
\tbnote{Es cierto, mejor, ya lo puse}.\\
...
\begin{align*}
|\beta(p)\rangle =& \; e^{is} \left( \dfrac{1}{2} |00\rangle - \dfrac{i}{2\sqrt{3}} |01\rangle - \dfrac{i}{2\sqrt{3}} |10 \rangle - \dfrac{i}{2\sqrt{3}} |11\rangle \right) \\
& + e^{-is} \left( \dfrac{1}{2} |00\rangle + \dfrac{i}{2\sqrt{3}} |01\rangle + \dfrac{i}{2\sqrt{3}} |10\rangle + \dfrac{i}{2\sqrt{3}} |11 \rangle \right),
\end{align*}
...
1PR circuit.
\cpnote{De nuevo, quizá aca convenga tener notacion de qubits y no de sistemas de 4 niveles}
\tbnote{Va, listo.}
...
\cpnote{Aca solo usas dos niveles, pero el caso general requiere 2 qubits. Ahora, si solo 
se requiere un qubit, pues está chido pero toca decirlo. Quizá dejarlo como un ultimo ejemplo. 
Toca citar acá tambien el trabajo de los brasileños... a ver si lo encuentro, Andrea 
Valdez me va a pasar la cita. De hecho pordemos leer el articulo para inspiracion y citar} \tbnote{Va, puse explícitamente lo de que sólo se usa un qubit más abajo} \tbnote{Tambien puse la cita del trabajo brasileño}
...
\end{align*}
with $\sin s= \sqrt{p}$\cpnote{Que tal si lo dejamos como $\sin s= \sqrt{p}$? Si estas de 
acuerdo igual cambia los siguientes} \tbnote{Va, ya los cambié}.
...
{\color{purple} Note that in this case we actually only need one ancilla qubit
...
{\color{purple} However, since a Pauli dynamical map should begin at the
...
\cpnote{Esta parte está desordenada. Porque parece qeu das el procedimiento 
en los 3 putnos de arriba y leugo que siempre no, qu evalta algo. Trata de iterar el paso 3 
estos dos parrafos para que tengan un flujo mas chido. }
\tbnote{Va, lo cambié y agregué esto al paso 3. Propongo quitar lo rojo y dejar lo morado}
To fulfill this additional condition,  we restrict the first row of
