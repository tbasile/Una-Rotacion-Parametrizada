...
%\tbnote{Obteniendo el seno y coseno de estos ángulos tampoco se 
%consigue algo mucho mejor:
%\begin{align*}
%\cos \theta_0 = \dfrac{\sqrt{k_{00} k_{10}} + \sqrt{k_{01} k_{11}}}{\sqrt{(k_{00} + k_{01}) (k_{10} + k_{11})}} \\
%\sin \theta_0 =  \dfrac{\sqrt{k_{01} k_{10}} - \sqrt{k_{00} k_{11}}}{\sqrt{(k_{00} + k_{01}) (k_{10} + k_{11})}}  \\
%\cos \theta_2 = \sqrt{\dfrac{k_{00}k_{10}}{(k_{00}+k_{01})(k_{10}+k_{11})}} \left( -1 + \sqrt{\dfrac{k_{01}k_{11}}{k_{00}k_{10}}} \right) \\
%\sin \theta_2 =  - \sqrt{\dfrac{k_{00}k_{10} + k_{01}k_{11} - 2 \sqrt{k_{00} k_{01} k_{10} k_{11}}}{(k_{00} + k_{01}) (k_{10} + k_{11})}} \left(\sqrt{\dfrac{k_{01}}{k_{00}}} + \sqrt{\dfrac{k_{11}}{k_{10}}} \right) \left(-1 + \sqrt{\dfrac{k_{01}k_{11}}{k_{00}k_{10}}} \right)^{-1}
%\end{align*}
%Probé combinar estas cantidades de diferentes formas, pero ninguna se simplificaba.}

\tbnote{Creo que la mejor forma es la siguiente (nota:
intercambié los ángulos 0 y 1 en la imagen para que 1 y 2
sean los que van juntos y el 0 por separado, en lugar de que los juntos sean el 0 y 2) :}
\tbnote{of angles defined by the following equations:
\begin{align*}
...
\end{align*}
También podría quitar los factores de 2 que hay aquí y meter factores de 2 en los ángulos del circuito.
}
...
\end{align*}
The rest of the operators $A$ and $B$ can be defined
