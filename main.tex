...
\section*{Abstract}
Pauli channels are fundamental in the context of quantum computing as they model noise in quantum devices. 
We propose a quantum algorithm for simulating Pauli channels on quantum computers 
...
\cpnote{Porfa 3 propuestas de titulo y un intento de abstract. Ese es mas facil que la intro!
de neuvo, quizá vale la pena que te einspires en otros abstracts de PlosOne}
\tbnote{Va, ya puse un abstract basado en lo que vi en articulos de PlosOne}
\tbnote{Para el título, tengo las siguientes propuestas:}
{\color{green}
\begin{itemize}
\item Quantum Algorithm for Simulation of Parametrized Pauli Channels.
\item Parametrized Pauli Channels Simulation in Quantum Computes.
\item Quantum Simulation of Pauli Channels: Algorithm, Implementation and 
generalization to parametrized channels.
\end{itemize}
}
...
which can be described by the theory of quantum
channels~\cite{cleve,Zanetti,Xin,Wei}. \tbnote{Cita a Andrea}
{\color{red} For example, this has been done using optical arrangements
to study the emergence of multipartite entanglement \cite{Andrea,Andrea_AD}.}
\cpnote{Se siente forzada. nos toca encontrar mas ejemplos e introducirla de
manera mas natural. Quizá mas adelante}
\tbnote{Va, es cierto.
Puedo incluir más trabajos y hablarlo más en general para que no se sienta forzado:}
{\color{purple}
...
and relevance in the context of quantum computing~\cite{Davalos,Zbigniew,geometry}, 
\cpnote{Aca citar nuestros trabajos con Davalos. } \tbnote{Ya agregué la cita}
as they serve as effective models for the noise affecting quantum devices~\cite{Flammia,Fawzi}.
\cpnote{Creo que la transición de general a particular va demasiado rapido. Quizá hacerle 
ver a Tomas y luego reescribirlo yo}
\tbnote{Va, sí lo veo, gracias!}

\cpnote{De nuevo una transicion muy abrupta. Para que ver los circuitos? Priero 
hablar de los circuitos, quizá una frase, luego hablar de los parametrizadso, 
Aca quiza citar a Barrero en innsbruk con sus vainas de enlazamiento}
\tbnote{Lo de Barreiro es una de las citas que agregué nuevas arriba}
\tbnote{Para lo de lo abrupto, quizá una frase como:} 
{\color{purple} To represent the algorithms
done in quantum computers, either
to simulate a physical system or for some other purpose,
one often uses a quantum circuit~\cite{chuangbook}.}
In this work, we shall also work with parametrized quantum circuits, that is,  
...
\cpnote{Me gustaron mucho estos dos ultimos parrafos. Croe que se puede ir mencionando las secciones
y poner la info del siguiente parrafo aca. Ahora, no lo pongas a lo bestia, sino realmente
integrando e hilando las ideas. Si hay preguntas, me dices que tos.}
\tbnote{Va, pongo una versión de estos dos últimos párrafos pero incluyendo lo de las secciones del siguiente aquí:}

{\color{purple}  
In this article,
after providing the definition of quantum
channels and $N$-qubit Pauli channels
in section \ref{sec: Pauli Channels},
 our first objective is to present a 
quantum algorithm capable of simulating Pauli channels on quantum computers.
We do this in section \ref{sec: Circuit for a Pauli Channel},
where we also demonstrate its implementation using IBM's quantum computers for 
the particular case of single-qubit Pauli channels. 

Expanding beyond discrete Pauli channels,
we introduce the concept of Pauli dynamical maps, 
defined as a continuous parametrization of
$N$-qubit Pauli channels. 
Therefore, in section \ref{sec: 1PR Circuits}
we shift our focus to study parametrized
quantum channels, 
aiming to
adapt the algorithm developed for channels 
to dynamical maps. 
Furthermore, we contribute to the body of work related to parametrized quantum circuits by establishing theorem \ref{theorem2},
which  sets the mathematical conditions for the transformations that 
can be done using a parametrized circuit with the condition that only one
 single-qubit operation in the circuit may depend on the parameter.
Finally, in section \ref{sec: 1PR circuit for a Pauli map},
we conclude about the 
Pauli dynamical maps that fulfill the conditions of theorem \ref{theorem2}. 
}

\tbnote{Y esto ya se va entonces}
{\color{red}
...
\cpnote{Iterar este parrafo cuando estemos cerca del final. Ta un tilin chafa.}
...
\fref{fig:curves1}. 
{\color{purple} The examples we show include some the most common maps: the bit flip, phase flip, bit-phase flip and depolarizing.
However, we also include a more uncommon map, which we call the parabolic dynamical map,
...
this curve of states can be \cpnote{supongo qeye hay muchas curvas. Es correcto eso? Quizá un ``this curve of states can be'' sea mas preciso.} 
\tbnote{Sí es correcto, va}
\begin{align*}
...
\cpnote{este mapa no es muy comun. Toca comentar algo al respecto. No se si acá o mas arriba. Si, ya se donde. En el parrafo antes de los ejemplos. } \tbnote{Va, listo, lo puse en morado}
\begin{eqnarray}
...
\item[3.] Finally, we choose a unitary matrix $V$ \cpnote{Esta supongo qeu 
la escoges aleatoriamente, y que la tomas por ahi del CUE. Lo platicamos y metemos 
los detalles. }
\cpnote{Sigo sin saber coo escoges la matriz V. Espero platicar contigo}
\tbnote{No, no lo hago con CUE ni nada así por lo de que el
primer renglon debe de tner una forma particular y no sabría cómo agregar eso tras
elegir una matriz de CUE.}
\tbnote{Lo que hago es empezar con ese renglon y busco un segundo renglón que sea ortogonal al primero y normalizado.
Para ello, tomo tres vectores $w_1, w_2, w_3$
normalizados y ortogonales al primer renglon.
Entonces, el segundo renglon será en general
de la forma $r_1 w_1 + r_2 w_2 + r_3 w_3$
con $r_i$ complejos y que $|r_1|^2 + |r_2|^2 + |r_3|^2 = 1$ (para que el 
segundo renglon esté normalizado).
Entonces, busco esos $r_1, r_2, r_3$ al azar  
(básicamente las partes reales e imaginarias
de las $r_i$ deben de estar en una 6-esfera,
por lo que escojo un punto al azar en dicha esfera).}
...
\cpnote{Creo qeu ahora está demasiado largo. jejej. hablemos y tratamos de hacer algo que se 
acomode bien a lo que ambos nos parezca.}
\tbnote{Jaja sí, va}\\

Finally, we choose a unitary matrix $V$ with the condition that its first
row is equal to $e^{i\theta} (|a|,|b|,|c|,0)$
with $\theta$ an arbitrary real number.
That way, we can define $|a\rangle = V |a'\rangle$, $|b\rangle = V |b'\rangle$, $|c\rangle = V |c'\rangle$
and since $V$ is unitary, these unprimed vectors will fulfill the conditions of \eref{eq:conditions-vecs}.
...
The choice of such a matrix $V$ can be done at random as follows:
After setting the first row,
construct three vectors $\vec{w}_1 , \vec{w}_2, \vec{w}_3$ orthogonal to it
using the Gram-Schmidt process.
Then select random complex numbers $r_1, r_2, r_3$ such
that $|r_1|^2 + |r_2|^2 + |r_3|^2 = 1$ and
define the second row of $V$ to be
$r_1 \vec{w}_1 + r_2 \vec{w}_2 + r_3 \vec{w}_3$.
...
\section{Conclusion} % {{{
\tbnote{Primera versión para ir teniendo algo escrito }\\


In this work, we focused on the analysis of Pauli channels in $N$-qubit systems and
...
{\color{purple}
Consequently, this work presents another example of the
simulation of open quantum systems, and their
...
{\color{purple} Therefore, it can be useful for simplifying any algorithm
that requires a parametrized quantum circuit,
such as those for quantum machine learning.}

\cpnote{Leiste como escribir una conclusion? Aca falta algo importante. Si ya lo leiste, 
dale una pensanda y si no, leelo a la luz de lo uqe escribiste. En todo caso, me parece una
muy buena primera iteracion para las conclusiones!! }
\tbnote{Ah, supongo que lo de volver a expandir y hablar en general para terminar de hacer la forma de ``hourglass''
en el artículo. Agregué algo en morado para eso.}

\tbnote{Mejor quitar todo lo anterior y dejar esto:}
